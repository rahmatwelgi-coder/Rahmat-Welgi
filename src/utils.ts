...
export function makeDefaultState(name = ""): AppState {
  ...
  return {
    ...
    activityLogs: [], // Clear default logs to avoid mismatching expenses
    ...
    profileAvatar: "⚡",
    profileColor: "#C9A84C",
    profileMotto: "Konsistensi adalah kunci keberhasilan!",
    streakFreezes: 1,
    customActiveDateEnabled: false,
    customActiveDate: toLocalDateStr(today),
    cycleStartDay: 1,
    banks: [
      { id: "b_1", name: "Bank Kesatu (Utama)", balance: 500000, icon: "🏦", notes: "Rekening transaksi harian utama", isCurrentTarget: true },
      { id: "b_2", name: "Bank Kedua (Tabungan)", balance: 300000, icon: "💳", notes: "Rekening tabungan", isCurrentTarget: false },
      { id: "b_3", name: "Bank Ketiga (Cadangan)", balance: 455000, icon: "💵", notes: "Rekening cadangan", isCurrentTarget: false }
    ]
  };
}

export function getCurrentCycleRange(todayStr: string, cycleStartDay: number) {
  const [year, month, day] = todayStr.split("-").map(Number);
  
  let startYear = year;
  let startMonth = month - 1; // 0-indexed month
  let endYear = year;
  let endMonth = month - 1;
  
  if (day >= cycleStartDay) {
    // Starts this calendar month, ends in next calendar month
    startMonth = month - 1;
    endMonth = month;
  } else {
    // Starts last calendar month, ends in this calendar month
    startMonth = month - 2;
    endMonth = month - 1;
  }
  
  const startDate = new Date(startYear, startMonth, cycleStartDay);
...
export function getPreviousCycleRange(todayStr: string, cycleStartDay: number) {
  const current = getCurrentCycleRange(todayStr, cycleStartDay);
  
  // Previous cycle start: 1 month before current start date
  const prevStart = new Date(current.startDate);
  prevStart.setMonth(prevStart.getMonth() - 1);
  
  // Previous cycle end: 1 day before current start date
  ...
  return {
    startDate: prevStart,
    endDate: prevEnd,
    startStr: toLocalDateStr(prevStart),
    endStr: toLocalDateStr(prevEnd)
  };
}

export function getExpensesForRange(state: AppState, startStr: string, endStr: string): { total: number; byCategory: Record<string, number> } {
    ...
    if (log.type === "expense" && log.timestamp && log.value) {
      ...
      if (logDateStr >= startStr && logDateStr <= endStr) {
        ...
        if (matchedCat) {
          ...
          const matchedCustom = (state.customExp || []).find(ce => actionLower.includes(ce.name.toLowerCase()));
          if (matchedCustom) {
            cat = matchedCustom.id;
          }
        }
        
        byCategory[cat] = (byCategory[cat] || 0) + log.value;
        total += log.value;
      }
    }
  });
...
export function getCycleExpenses(state: AppState): { total: number; byCategory: Record<string, number> } {
  ...
  const { total, byCategory } = getExpensesForRange(state, startStr, endStr);
  
  if (total === 0 && cycleStart === 1) {
    const baseExpenses = { ...(state.expenses || {}) };
    const customExpensesSum = (state.customExp || []).reduce((sum, ce) => {
      baseExpenses[ce.id] = ce.amt;
      return sum + ce.amt;
    }, 0);
    const standardSum = Object.values(state.expenses || {}).reduce((sum, val) => sum + (val || 0), 0);
    return {
      total: standardSum + customExpensesSum,
      byCategory: baseExpenses
    };
  }
