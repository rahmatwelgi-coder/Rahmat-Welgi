...
import { fmtRp, fmtK, totalExp, totalWkXP, totalHabXP, getLv, makeDefaultState, getCycleExpenses, getCurrentCycleRange, toLocalDateStr } from "./utils";
...
export function getTodayStr(state: AppState | null) {
  if (state && state.customActiveDateEnabled && state.customActiveDate) {
    return state.customActiveDate;
  }
...
export default function App() {
  ...
  const handleStateChange = (updates: Partial<AppState>) => {
    ...
    if (updates.expenses && updates.activityLogs === undefined) {
    ...
    if (updates.customExp && updates.activityLogs === undefined) {
    ...
    // Auto-update goals yang linked ke workout
    if (updates.workouts && state.goals) {
      const linkedWorkoutGoals = state.goals.filter(
        g => g.linkedMetric?.type === "workout_sessions" ||
             g.linkedMetric?.type === "workout_distance_km"
      );
      if (linkedWorkoutGoals.length > 0) {
        const updatedGoalsFromWorkout = [...(updates.goals || state.goals)];
        Object.entries(updates.workouts).forEach(([dateStr, log]) => {
          const prevLog = state.workouts?.[dateStr];
          // Hanya proses jika ini entry BARU atau berubah, dan bukan rest day
          if (log && log.type !== "rest" && JSON.stringify(prevLog) !== JSON.stringify(log)) {
            linkedWorkoutGoals.forEach(g => {
              const filterType = g.linkedMetric?.workoutTypeFilter;
              if (filterType && filterType !== log.type) return;
              if (g.linkedMetric?.type === "workout_sessions" && !prevLog) {
                // Hanya tambah sesi jika sebelumnya belum ada log di tanggal ini
                const idx = updatedGoalsFromWorkout.findIndex(x => x.id === g.id);
                if (idx !== -1) {
                  const newProg = Math.min(
                    updatedGoalsFromWorkout[idx].target,
                    updatedGoalsFromWorkout[idx].progress + 1
                  );
                  updatedGoalsFromWorkout[idx] = {
                    ...updatedGoalsFromWorkout[idx],
                    progress: newProg,
                    completed: newProg >= updatedGoalsFromWorkout[idx].target
                  };
                }
              }
              if (g.linkedMetric?.type === "workout_distance_km" && log.type === "cardio") {
                const idx = updatedGoalsFromWorkout.findIndex(x => x.id === g.id);
                if (idx !== -1) {
                  const distanceDelta = (log.reps || 0) - (prevLog?.reps || 0);
                  if (distanceDelta > 0) {
                    const newProg = Math.min(
                      updatedGoalsFromWorkout[idx].target,
                      updatedGoalsFromWorkout[idx].progress + distanceDelta
                    );
                    updatedGoalsFromWorkout[idx] = {
                      ...updatedGoalsFromWorkout[idx],
                      progress: newProg,
        ...
        updates.goals = updatedGoalsFromWorkout;
      }
    }

    // Auto-update goals yang linked ke expense savings
    if ((updates.expenses || updates.customExp || updates.budget) && state.goals) {
      const linkedExpenseGoals = state.goals.filter(
        g => g.linkedMetric?.type === "expense_savings"
      );
      if (linkedExpenseGoals.length > 0) {
        ...
        const prevSnapshot = state.expenseHistory?.[prevMonthKey];
        const prevBudget = prevSnapshot ? prevSnapshot.budget : state.budget;

        const nextExpenses = updates.expenses !== undefined ? updates.expenses : state.expenses;
        const nextCustomExp = updates.customExp !== undefined ? updates.customExp : state.customExp;
        
        const baseExpenses = Object.values(nextExpenses || {}).reduce((a: number, v: any) => a + (Number(v) || 0), 0);
        const customExpenses = (nextCustomExp || []).reduce((a: number, e: any) => a + (Number(e?.amt) || 0), 0);
        const totalSpentThisMonth = baseExpenses + customExpenses;

        const savings = prevBudget - totalSpentThisMonth;
        const savingsProgress = savings > 0 ? savings : 0;

        const updatedGoalsFromExpense = [...(updates.goals || state.goals)];
        linkedExpenseGoals.forEach(g => {
          const idx = updatedGoalsFromExpense.findIndex(x => x.id === g.id);
          if (idx !== -1) {
            const newProg = Math.min(
              updatedGoalsFromExpense[idx].target,
              savingsProgress
            );
            updatedGoalsFromExpense[idx] = {
              ...updatedGoalsFromExpense[idx],
              progress: newProg,
              completed: newProg >= updatedGoalsFromExpense[idx].target
            };
          }
        });
        updates.goals = updatedGoalsFromExpense;
      }
    }
    ...
    if (newLogs.length > 0) {
      updates.activityLogs = [
        ...(updates.activityLogs !== undefined ? updates.activityLogs : (state.activityLogs || [])),
        ...newLogs
      ];
    }
  ...
  const shiftCustomDate = (days: number) => {
    if (!state || !state.customActiveDate) return;
    const current = new Date(state.customActiveDate);
    current.setDate(current.getDate() + days);
    const newDateStr = toLocalDateStr(current);
    handleStateChange({ customActiveDate: newDateStr });
  };

  // Automated day-transition checking and archiving
  useEffect(() => {
    if (!state || !user) return;
    const todayStr = getTodayStr(state);
  ...
  const totalSpend = state.cycleStartDay && state.cycleStartDay !== 1 ? getCycleExpenses(state).total : totalExp(state);
  ...
  const getCyclePeriodText = () => {
    ...
    if (cycleStart === 1) {
      return `${getMonthName(state.month, state.lang || "id")} ${state.year}`;
    }
    const { startDate, endDate } = getCurrentCycleRange(todayStr, cycleStart);
    const options: Intl.DateTimeFormatOptions = { day: "numeric", month: "short" };
    const startText = startDate.toLocaleDateString(state.lang === "en" ? "en-US" : "id-ID", options);
    const endText = endDate.toLocaleDateString(state.lang === "en" ? "en-US" : "id-ID", { day: "numeric", month: "short", year: "numeric" });
    return `${startText} - ${endText}`;
  };

  const activeStyleClass = `style-${state.visualStyle || "modern"}`;

  const getBgClass = () => {
    const isDark = state.theme === "dark";
    const bg = state.bgTheme || "classic";
    if (isDark) {
      switch (bg) {
        case "navy": return "dark bg-[#0A0E17] text-zinc-100";
        case "emerald": return "dark bg-[#070C08] text-zinc-100";
        case "cosmic": return "dark bg-[#0B0912] text-zinc-100";
        case "black": return "dark bg-black text-zinc-50";
        default: return "dark bg-zinc-950 text-zinc-100";
      }
    } else {
      switch (bg) {
        case "mint": return "bg-[#F0F5F1] text-zinc-800";
        case "lavender": return "bg-[#F3F1F6] text-zinc-800";
        case "sunset": return "bg-[#FAF1EE] text-zinc-800";
        case "frost": return "bg-[#F1F4F7] text-zinc-800";
        default: return "bg-[#FAF9F6] text-zinc-800";
      }
    }
  };
  ...
  return (
    <div 
      className={`min-h-screen relative pb-16 transition-all duration-300 ${activeStyleClass} ${getBgClass()}`}
      style={state.customBgImage ? { backgroundImage: `url(${state.customBgImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' } : undefined}
    >
      {/* Background Image Semi-Transparent Overlay for Legibility */}
      {state.customBgImage && (
        <div className="fixed inset-0 pointer-events-none z-0 bg-white/50 dark:bg-zinc-950/75 backdrop-blur-[0.5px]" />
      )}
      ...
      <header className="sticky top-0 z-40 bg-white/80 dark:bg-zinc-900/85 backdrop-blur-md border-b border-zinc-200/50 dark:border-zinc-800/80 px-4 py-3 shadow-[0_1px_3px_rgba(0,0,0,0.01)]">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div 
              className="w-10 h-10 rounded-full flex items-center justify-center text-xl shadow-[0_4px_12px_rgba(0,0,0,0.06)] border border-zinc-200/20 overflow-hidden select-none"
              style={{ backgroundColor: state.profileColor || "#C9A84C" }}
            >
              {state.profileImage ? (
                <img src={state.profileImage} alt="Profile" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              ) : (
                state.profileAvatar || "⚡"
              )}
            </div>
            <div>
              ...
              <div className="text-[9px] text-zinc-400 dark:text-zinc-500 tracking-widest uppercase font-extrabold mt-0.5">
                {getCyclePeriodText()}
                {state.profileMotto && (
                  <span className="hidden md:inline-block text-zinc-400 dark:text-zinc-500 font-medium normal-case italic ml-2 bg-zinc-100/50 dark:bg-zinc-800/50 px-2 py-0.5 rounded-md">
                    "{state.profileMotto}"
                  </span>
                )}
              </div>
      ...
      <main className="max-w-4xl mx-auto p-4 relative z-10">
        {/* SIMULATED DATE BANNER */}
        {state.customActiveDateEnabled && state.customActiveDate && (
          <div className="mb-6 bg-amber-50/90 dark:bg-amber-950/20 border-2 border-dashed border-amber-300 dark:border-amber-800/80 rounded-3xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-md animate-fade-in relative z-10">
            <div className="flex items-center gap-3">
              <span className="text-2xl animate-bounce">📅</span>
              <div>
                <div className="text-xs font-black text-amber-800 dark:text-amber-300 uppercase tracking-wide">
                  {state.lang === "en" ? "Date Simulation Mode Active" : "Mode Simulasi Tanggal Aktif"}
                </div>
                <div className="text-xs font-bold text-zinc-600 dark:text-zinc-400 mt-0.5">
                  {state.lang === "en" ? "App treats this date as Today:" : "Aplikasi mendeteksi tanggal ini sebagai Hari Ini:"}{" "}
                  <span className="font-mono font-black text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 rounded">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="bg-white dark:bg-zinc-800 border border-amber-200 dark:border-amber-900 hover:bg-amber-100/50 dark:hover:bg-amber-900/30 text-amber-800 dark:text-amber-300 font-extrabold px-3 py-1.5 rounded-xl text-xs transition-colors cursor-pointer"
              >
                ⬅️ -1 Hari
              </button>
              <input
                ...
                onChange={(e) => {
                  if (e.target.value) {
                    handleStateChange({ customActiveDate: e.target.value });
                    showToast(state.lang === "en" ? `📅 Date set to: ${e.target.value}` : `📅 Tanggal diatur ke: ${e.target.value}`);
                  }
                }}
                className="bg-white dark:bg-zinc-800 border border-amber-300 dark:border-amber-900 rounded-xl px-2.5 py-1 text-xs font-mono font-bold outline-none text-amber-800 dark:text-amber-300 cursor-pointer"
              />
              <button
                onClick={() => shiftCustomDate(1)}
                className="bg-white dark:bg-zinc-800 border border-amber-200 dark:border-amber-900 hover:bg-amber-100/50 dark:hover:bg-amber-900/30 text-amber-800 dark:text-amber-300 font-extrabold px-3 py-1.5 rounded-xl text-xs transition-colors cursor-pointer"
