...
import { fmtRp, fmtK, daysInMonth, totalWkXP, toLocalDateStr } from "../utils";
...
export default function WorkoutTab({ state, onChange, showToast }: WorkoutTabProps) {
  ...
  // Calculate current and previous calendar month workouts
  ...
  const currentMonthWorkouts = Object.entries(state.workouts || {})
  ...
  const wDone = currentMonthWorkouts.filter((w) => w.type !== "rest").length;
  const wMnt = currentMonthWorkouts.reduce((a, w) => a + (w.dur || 0), 0);
  const xp = currentMonthWorkouts.reduce((acc, w) => {
    const customXp = state.customWkXP?.[w.type];
    if (customXp !== undefined) return acc + customXp;
    const tDef = WTYPES.find((x) => x.id === w.type);
    return acc + (tDef?.xp || 0);
  }, 0);

  const prevWDone = prevMonthWorkouts.filter((w) => w.type !== "rest").length;
  const prevWMnt = prevMonthWorkouts.reduce((a, w) => a + (w.dur || 0), 0);
  const prevXp = prevMonthWorkouts.reduce((acc, w) => {
    const customXp = state.customWkXP?.[w.type];
    if (customXp !== undefined) return acc + customXp;
    const tDef = WTYPES.find((x) => x.id === w.type);
    return acc + (tDef?.xp || 0);
  ...
  const sessionsDiff = wDone - prevWDone;
  const durDiff = wMnt - prevWMnt;
  ...
  const handleSaveWorkout = () => {
    ...
    if (selDate === toLocalDateStr(new Date())) {
        ...
        {[
          {
            ...
            comp: isEn ? `Prev: ${prevWDone} (${sessionsDiff >= 0 ? `+${sessionsDiff}` : sessionsDiff})` : `Lalu: ${prevWDone} (${sessionsDiff >= 0 ? `+${sessionsDiff}` : sessionsDiff})`,
            compColor: sessionsDiff > 0 ? "text-emerald-600" : sessionsDiff < 0 ? "text-rose-600" : "text-zinc-400"
          },
          {
            i: "⏱️",
            v: `${wMnt}${isEn ? " min" : " m"}`,
            l: isEn ? "Total Minutes" : "Total Menit",
            c: "text-emerald-600",
            bg: "bg-emerald-50/30 border-emerald-100",
            comp: isEn ? `Prev: ${prevWMnt}m (${durDiff >= 0 ? `+${durDiff}` : durDiff}m)` : `Lalu: ${prevWMnt}m (${durDiff >= 0 ? `+${durDiff}` : durDiff}m)`,
            compColor: durDiff > 0 ? "text-emerald-600" : durDiff < 0 ? "text-rose-600" : "text-zinc-400"
          },
          {
            i: "⭐",
            v: `${xp} XP`,
            ...
            comp: isEn ? `Prev: ${prevXp} (${xpDiff >= 0 ? `+${xpDiff}` : xpDiff})` : `Lalu: ${prevXp} (${xpDiff >= 0 ? `+${xpDiff}` : xpDiff})`,
            compColor: xpDiff > 0 ? "text-emerald-600" : xpDiff < 0 ? "text-rose-600" : "text-zinc-400"
          },
          {
            i: "🔥",
            v: state.streak || 0,
            ...
            comp: isEn ? "Daily Rolling" : "Berjalan Harian",
            compColor: "text-zinc-400"
          }
        ].map((k, i) => (
          <div key={i} className={`bg-white border border-zinc-200/50 rounded-2xl p-4 shadow-sm text-center ${k.bg} transition-all duration-300 hover:scale-[1.02] flex flex-col justify-between min-h-[110px]`}>
            <div>
              <div className="text-2xl mb-1">{k.i}</div>
              <div className={`text-base font-extrabold font-mono ${k.c}`}>
                {k.v}
              </div>
              <div className="text-[9px] text-zinc-400 font-extrabold uppercase tracking-wider mt-1 block">{k.l}</div>
            </div>
            {k.comp && (
              <div className={`text-[9px] font-black font-mono mt-2 pt-1 border-t border-zinc-100/60 ${k.compColor}`}>
                {k.comp}
              </div>
            )}
          </div>
