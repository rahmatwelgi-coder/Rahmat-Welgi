...
import { fmtRp, fmtK, totalExp, totalWkXP, getCycleExpenses, getCurrentCycleRange, getPreviousCycleRange, getExpensesForRange, toLocalDateStr } from "../utils";
...
export default function ReportTab({ state }: ReportTabProps) {
  ...
  const todayKey = state.customActiveDateEnabled && state.customActiveDate ? state.customActiveDate : toLocalDateStr(new Date());

  const hasCustomCycle = state.cycleStartDay && state.cycleStartDay !== 1;
  
  let activeExpensesMap: Record<string, number> = {};
  let prevExpensesMap: Record<string, number> = {};
  let currentTotalExpense = 0;
  
  let currentPeriodLabel = `${currentMonthName} ${state.year}`;
  let prevPeriodLabel = `${prevMonthName} ${prevMonthYear}`;
  ...
  if (hasCustomCycle) {
    ...
    // Current custom cycle range
    const currentRange = getCurrentCycleRange(todayStr, cycleStart);
    const currCycle = getExpensesForRange(state, currentRange.startStr, currentRange.endStr);
    activeExpensesMap = currCycle.byCategory;
    currentTotalExpense = currCycle.total;
    
    // Label for current cycle
    const optionsShort: Intl.DateTimeFormatOptions = { day: "numeric", month: "short" };
    currentPeriodLabel = `${currentRange.startDate.toLocaleDateString(state.lang === "en" ? "en-US" : "id-ID", optionsShort)} - ${currentRange.endDate.toLocaleDateString(state.lang === "en" ? "en-US" : "id-ID", { day: "numeric", month: "short", year: "2-digit" })}`;
    
    // Previous custom cycle range
    const prevRange = getPreviousCycleRange(todayStr, cycleStart);
    const prevCycle = getExpensesForRange(state, prevRange.startStr, prevRange.endStr);
    prevExpensesMap = prevCycle.byCategory;
    
    // Label for previous cycle
    prevPeriodLabel = `${prevRange.startDate.toLocaleDateString(state.lang === "en" ? "en-US" : "id-ID", optionsShort)} - ${prevRange.endDate.toLocaleDateString(state.lang === "en" ? "en-US" : "id-ID", { day: "numeric", month: "short", year: "2-digit" })}`;
  } else {
    // Normal calendar month behavior
    activeExpensesMap = state.expenses || {};
    currentTotalExpense = totalExp(state);
    
    const prevMonthSnap = state.expenseHistory?.[prevMonthKey];
    prevExpensesMap = prevMonthSnap?.expenses || {};
  }

  const currentCustom = state.customExp || [];
  const prevCustom = hasCustomCycle ? [] : (state.expenseHistory?.[prevMonthKey]?.customExp || []);
  
  // Combine all custom expenses
  const allCustomExpenses = [
    ...(state.customExp || []),
    ...(state.expenseHistory?.[prevMonthKey]?.customExp || [])
  ];
  const uniqueCustomExpenses = Array.from(new Map(allCustomExpenses.map(ce => [ce.id, ce])).values());
  ...
  const categoriesData = [
    ...ECATS.map((c) => {
      const currentVal = activeExpensesMap?.[c.id] || 0;
      const prevVal = prevExpensesMap?.[c.id] || 0;
    ...
    ...uniqueCustomExpenses.map((ce) => {
      const currentVal = activeExpensesMap?.[ce.id] || 0;
      const prevVal = prevExpensesMap?.[ce.id] || 0;
      return {
        id: ce.id,
        name: ce.name.slice(0, 8),
        fullName: ce.name,
        icon: ce.icon || "📦",
        color: "#888",
        current: currentVal,
  ...
  const expenseComparisonChartData = categoriesData
    ...
    .map((c) => ({
      name: c.name,
      [currentPeriodLabel]: c.current,
      [prevPeriodLabel]: c.prev,
    }));
  ...
  // Get previous month's workout logs for calendar comparison
  const prevMonthPrefix = `${prevMonthYear}-${String(prevMonthIndex + 1).padStart(2, "0")}`;
  const prevMonthWorkouts = Object.entries(state.workouts || {})
    .filter(([dateKey]) => dateKey.startsWith(prevMonthPrefix))
    .map(([dateKey, log]) => ({ dateKey, ...log }));

  const prevTotalSessions = prevMonthWorkouts.filter((w) => w.type !== "rest").length;
  const prevTotalDuration = prevMonthWorkouts.reduce((acc, w) => acc + (w.dur || 0), 0);
  const prevWorkoutXPMonth = prevMonthWorkouts.reduce((acc, w) => {
    const customXp = state.customWkXP?.[w.type];
    if (customXp !== undefined) return acc + customXp;
    const t = WTYPES.find((x) => x.id === w.type);
    return acc + (t?.xp || 0);
  ...
  const sessionsDiff = totalSessions - prevTotalSessions;
  ...
  // Previous month comparison insights
  if (sessionsDiff > 0) {
    workoutInsights.push(
      isEn
        ? `📈 **Workout frequency increased!** You did **${sessionsDiff} more sessions** compared to last month (${prevTotalSessions} sessions). Great progress!`
        : `📈 **Frekuensi workout meningkat!** Kamu melakukan **${sessionsDiff} sesi lebih banyak** dibanding bulan lalu (${prevTotalSessions} sesi). Progres yang bagus!`
    );
  } else if (sessionsDiff < 0) {
    workoutInsights.push(
      isEn
        ? `📉 **Workout frequency decreased.** You did **${Math.abs(sessionsDiff)} fewer sessions** compared to last month (${prevTotalSessions} sessions). Let's catch up!`
        : `📉 **Frekuensi workout menurun.** Kamu melakukan **${Math.abs(sessionsDiff)} sesi lebih sedikit** dibanding bulan lalu (${prevTotalSessions} sesi). Yuk kejar lagi!`
    );
  } else {
    workoutInsights.push(
      isEn
        ? `🔄 **Steady Pace:** You maintained the same number of workout sessions as last month (**${totalSessions} sessions**).`
        : `🔄 **Tempo Stabil:** Kamu mempertahankan jumlah sesi workout yang sama dengan bulan lalu (**${totalSessions} sesi**).`
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in text-zinc-700 pb-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-zinc-200/60 pb-5 mb-5">
          <div>
            ...
            <p className="text-[10px] text-zinc-400 uppercase tracking-widest font-black mt-1">
              {isEn ? "Performance Summary & Analysis" : "Rangkuman Performa & Analisis"} &bull; {hasCustomCycle ? currentPeriodLabel : `${currentMonthName} ${state.year}`}
            </p>
          </div>
      ...
      <div className="bg-white border border-zinc-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-3xl p-6 space-y-5">
        <div className="flex justify-between items-center border-b border-zinc-100 pb-3">
          ...
          <span className="text-[9px] text-zinc-400 font-extrabold uppercase font-mono">
            {hasCustomCycle ? (isEn ? "Active Cycle vs Previous" : "Siklus Aktif vs Sebelumnya") : `${isEn ? "This Month vs" : "Bulan Ini vs"} ${prevMonthName}`}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
          ...
          <div className="md:col-span-5 space-y-4">
            ...
            <div className="bg-zinc-50 border border-zinc-200/60 rounded-2xl p-4">
              <span className="text-[9px] text-zinc-400 font-extrabold uppercase tracking-wider block">{hasCustomCycle ? (isEn ? "Previous Cycle Comparison" : "Perbandingan Siklus Sebelumnya") : `${isEn ? "Last Month Comparison" : "Perbandingan Bulan Lalu"} (${prevMonthName})`}</span>
              ...
              <span className="text-[9px] text-zinc-400 font-bold block mt-1.5">
                {expenseDiff <= 0 ? (isEn ? "Saved" : "Hemat") : (isEn ? "Overspent" : "Boros")} {isEn ? "by" : "sebesar"} {fmtRp(Math.abs(expenseDiff))} {hasCustomCycle ? (isEn ? "compared to previous cycle." : "dibanding siklus sebelumnya.") : (isEn ? "compared to last month." : "dibanding bulan lalu.")}
              </span>
            </div>
            ...
            <div className="bg-zinc-50 border border-zinc-200/60 rounded-2xl p-4">
              ...
              <span className="text-lg font-black text-emerald-600 font-mono mt-1 block">
                {fmtRp(activeExpensesMap.tabungan || 0)}
              </span>
            </div>
          </div>
              ...
              <div className="flex-1 min-h-0">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={expenseComparisonChartData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                    ...
                    <Bar dataKey={currentPeriodLabel} fill="#C9A84C" radius={[2, 2, 0, 0]} />
                    <Bar dataKey={prevPeriodLabel} fill="#e4e4e7" radius={[2, 2, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ) : (
      ...
      <div className="bg-white border border-zinc-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-3xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
          <div className="bg-zinc-50 border border-zinc-200/60 rounded-2xl p-4 flex flex-col justify-between min-h-[96px]">
            <div>
            ...
            <span className="text-[8px] font-black font-mono text-zinc-400 block mt-1.5 border-t border-zinc-100 pt-1">
              {isEn ? "Prev" : "Lalu"}: {prevTotalSessions} ({sessionsDiff >= 0 ? `+${sessionsDiff}` : sessionsDiff})
            </span>
          </div>
          <div className="bg-zinc-50 border border-zinc-200/60 rounded-2xl p-4 flex flex-col justify-between min-h-[96px]">
            <div>
            ...
            <span className="text-[8px] font-black font-mono text-zinc-400 block mt-1.5 border-t border-zinc-100 pt-1">
              {isEn ? "Prev" : "Lalu"}: {prevTotalDuration}m ({totalDuration - prevTotalDuration >= 0 ? `+${totalDuration - prevTotalDuration}` : totalDuration - prevTotalDuration}m)
            </span>
          </div>
          <div className="bg-zinc-50 border border-zinc-200/60 rounded-2xl p-4 flex flex-col justify-between min-h-[96px]">
            <div>
            ...
            <span className="text-[8px] font-black font-mono text-zinc-400 block mt-1.5 border-t border-zinc-100 pt-1">
              {isEn ? "Prev" : "Lalu"}: {prevWorkoutXPMonth} ({workoutXPMonth - prevWorkoutXPMonth >= 0 ? `+${workoutXPMonth - prevWorkoutXPMonth}` : workoutXPMonth - prevWorkoutXPMonth})
            </span>
          </div>
          <div className="bg-zinc-50 border border-zinc-200/60 rounded-2xl p-4 flex flex-col justify-between min-h-[96px]">
            <div>
            ...
            <span className="text-[8px] font-black font-mono text-zinc-400 block mt-1.5 border-t border-zinc-100 pt-1">
              {isEn ? "Calendar Month Only" : "Hanya Bulan Kalender"}
            </span>
          </div>
        </div>
