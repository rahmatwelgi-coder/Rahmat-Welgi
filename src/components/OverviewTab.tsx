...
import { fmtRp, fmtK, totalExp, totalWkXP, getLv, getCycleExpenses, getCurrentCycleRange, toLocalDateStr } from "../utils";
...
export default function OverviewTab({ state, onChange, showToast }: OverviewTabProps) {
  ...
  useEffect(() => {
    if (state.banks) {
      ...
      state.banks.forEach((b) => {
        initial[b.id] = String(b.balance);
      });
      setBankBalances(initial);
    }
  }, [state.banks]);

  const sumBals = Object.values(bankBalances).reduce((sum: number, val: any) => sum + (parseInt(val) || 0), 0) as number;
  ...
  const calculateCrossInsights = () => {
    const insights: { icon: string; text: string; type: "positive" | "negative" | "neutral" }[] = [];

    // Group the value of expense logs of "hiburan" and "jajanan" by YYYY-MM-DD date
    const dailyExpenseMap: Record<string, number> = {};
    (state.activityLogs || []).forEach(log => {
      if (log.type === "expense" && log.timestamp && log.value) {
        const dateStr = log.timestamp.slice(0, 10);
        const actionLower = log.action.toLowerCase();
        if (actionLower.includes("hiburan") || actionLower.includes("jajanan") || actionLower.includes("kategori hiburan") || actionLower.includes("kategori jajanan")) {
          dailyExpenseMap[dateStr] = (dailyExpenseMap[dateStr] || 0) + log.value;
        }
      }
    });

    // 30 days ago YYYY-MM-DD keys
    const last30Days: string[] = [];
    for (let i = 0; i < 30; i++) {
      const d = new Date();
      d.setDate(d.getDate() - i);
      last30Days.push(toLocalDateStr(d));
    }

    // a) Workout vs Jajanan/Hiburan
    const workoutExpenses: number[] = [];
    const restExpenses: number[] = [];

    last30Days.forEach(date => {
      const logWk = state.workouts?.[date];
      const hasWorkout = logWk && logWk.type !== "rest";
      const expVal = dailyExpenseMap[date] || 0;
      if (hasWorkout) {
        workoutExpenses.push(expVal);
      } else {
        restExpenses.push(expVal);
      }
    });

    if (workoutExpenses.length >= 5 && restExpenses.length >= 5) {
      const avgWorkoutExp = workoutExpenses.reduce((a, b) => a + b, 0) / workoutExpenses.length;
      const avgRestExp = restExpenses.reduce((a, b) => a + b, 0) / restExpenses.length;

      if (avgRestExp > 0 || avgWorkoutExp > 0) {
        if (avgWorkoutExp < avgRestExp) {
          const percent = Math.round(((avgRestExp - avgWorkoutExp) / (avgRestExp || 1)) * 100);
          if (percent > 0) {
            insights.push({
              icon: "💪",
              text: isEn 
                ? `Di hari kamu workout, pengeluaran jajanan/hiburan rata-rata ${percent}% lebih rendah.` 
                : `Di hari kamu workout, pengeluaran jajanan/hiburan rata-rata ${percent}% lebih rendah.`,
              type: "positive"
            });
          }
        } else if (avgWorkoutExp > avgRestExp) {
          const percent = Math.round(((avgWorkoutExp - avgRestExp) / (avgRestExp || 1)) * 100);
          if (percent > 0) {
            insights.push({
              icon: "💪",
              text: isEn 
                ? `Di hari kamu workout, pengeluaran jajanan/hiburan rata-rata ${percent}% lebih tinggi.` 
                : `Di hari kamu workout, pengeluaran jajanan/hiburan rata-rata ${percent}% lebih tinggi.`,
              type: "negative"
            });
          }
    ...
    // b) Habit Completion vs Workout
    const totalHabits = state.habits?.length || 0;
    const getHabitCompletionRate = (dateStr: string) => {
      if (totalHabits === 0) return 0;
      const todayStr = toLocalDateStr(new Date());
      const checkedCount = dateStr === todayStr
        ? Object.keys(state.checkedToday || {}).length
        : (state.habitHistory?.[dateStr] || []).length;
      return (checkedCount / totalHabits) * 100;
    };

    if (totalHabits > 0) {
      const workoutHabitRates: number[] = [];
      const restHabitRates: number[] = [];

      last30Days.forEach(date => {
        const logWk = state.workouts?.[date];
        const hasWorkout = logWk && logWk.type !== "rest";
        const rate = getHabitCompletionRate(date);
        if (hasWorkout) {
          workoutHabitRates.push(rate);
        } else {
          restHabitRates.push(rate);
        }
      });

      if (workoutHabitRates.length >= 5 && restHabitRates.length >= 5) {
        const avgWorkoutHabit = workoutHabitRates.reduce((a, b) => a + b, 0) / workoutHabitRates.length;
        const avgRestHabit = restHabitRates.reduce((a, b) => a + b, 0) / restHabitRates.length;

        const diff = avgWorkoutHabit - avgRestHabit;
        if (Math.abs(diff) >= 1) {
          ...
          if (diff > 0) {
            insights.push({
              icon: "🔥",
              text: isEn 
                ? `Habit completion rate kamu ${percent}% lebih tinggi di hari yang ada sesi workout.` 
                : `Habit completion rate kamu ${percent}% lebih tinggi di hari yang ada sesi workout.`,
              type: "positive"
            });
          } else {
            insights.push({
              icon: "🔥",
              text: isEn 
                ? `Habit completion rate kamu ${percent}% lebih rendah di hari yang ada sesi workout.` 
                : `Habit completion rate kamu ${percent}% lebih rendah di hari yang ada sesi workout.`,
              type: "negative"
            });
          }
    ...
    // c) WoW Trend Last 7 Days vs Previous 7 Days
    if (totalHabits > 0) {
      const thisWeekHabitRate = thisWeekKeys.reduce((acc, key) => acc + getHabitCompletionRate(key), 0) / 7;
      const prevWeekHabitRate = prevWeekKeys.reduce((acc, key) => acc + getHabitCompletionRate(key), 0) / 7;

      const thisWeekFunExp = thisWeekKeys.reduce((acc, key) => acc + (dailyExpenseMap[key] || 0), 0);
      const prevWeekFunExp = prevWeekKeys.reduce((acc, key) => acc + (dailyExpenseMap[key] || 0), 0);

      if (thisWeekHabitRate < prevWeekHabitRate && thisWeekFunExp > prevWeekFunExp) {
        insights.push({
          icon: "⚠️",
          text: isEn 
            ? `7 hari terakhir, habit completion turun bersamaan dengan kenaikan pengeluaran hiburan — kemungkinan ada hubungan stres/mood.` 
            : `7 hari terakhir, habit completion turun bersamaan dengan kenaikan pengeluaran hiburan — kemungkinan ada hubungan stres/mood.`,
          type: "negative"
        });
      }
  ...
  const crossInsights = calculateCrossInsights();
  ...
  const { startStr, endStr } = getCurrentCycleRange(todayStr, cycleStart);
  ...
  let dLeft = 0;
  if (state.cycleStartDay && state.cycleStartDay !== 1) {
    const { endDate } = getCurrentCycleRange(todayStr, state.cycleStartDay);
    const [ty, tm, td] = todayStr.split("-").map(Number);
    const todayObj = new Date(ty, tm - 1, td, 0, 0, 0, 0);
    const endObj = new Date(endDate);
    endObj.setHours(23, 59, 59, 999);
    const diffTime = endObj.getTime() - todayObj.getTime();
    dLeft = Math.max(0, Math.ceil(diffTime / (1000 * 60 * 60 * 24)));
  } else {
    const todayDate = new Date().getDate();
    dLeft = Math.max(0, dim - todayDate);
  }
  ...
  const pieData = ECATS.filter((c) => c.id !== "tabungan" && (activeExpensesMap?.[c.id] || 0) > 0).map((c) => ({
    name: c.label.split(" ")[0],
    value: activeExpensesMap?.[c.id] || 0,
    color: c.color,
  }));
  ...
  const expenseBarData = ECATS.filter((c) => c.id !== "tabungan" && (activeExpensesMap?.[c.id] || 0) > 0).map((c) => ({
    name: c.label.split(" ")[0],
  ...
  budgetTips.push(isEn
    ? `This month's savings are safely locked: ${fmtRp(activeExpensesMap?.tabungan || 0)}.`
    : `Tabungan bulan ini aman terkunci: ${fmtRp(activeExpensesMap?.tabungan || 0)}.`);
  ...
  return (
    <div className="space-y-6 animate-fade-in text-zinc-700 pb-4">
      ...
      {/* Insight Korelasi Section */}
      <div className="bg-white border border-zinc-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-3xl p-6 mt-6">
        <div className="border-b border-zinc-100 pb-4 mb-4">
          <h3 className="text-xs font-black uppercase tracking-widest text-[#B8860B] flex items-center gap-2">
            🔗 {isEn ? "Correlation Insights" : "Insight Korelasi"}
          </h3>
          <p className="text-[10px] text-zinc-400 mt-1">
            {isEn ? "Relationship between habits, training, and your finances" : "Hubungan antar kebiasaan, latihan, dan keuangan kamu"}
          </p>
        </div>
        ...
        {crossInsights.length > 0 ? (
          <div className="space-y-3">
            {crossInsights.map((insight, index) => (
              <div
                key={index}
                className={`flex gap-3 items-start p-3 rounded-2xl border-l-4 bg-zinc-50/50 ${
                  insight.type === "positive"
                    ? "border-emerald-500 bg-emerald-50/10"
                    : insight.type === "negative"
                    ? "border-rose-500 bg-rose-50/10"
                    : "border-zinc-400 bg-zinc-50/10"
                }`}
              >
          ...
          <div className="flex flex-col items-center justify-center py-6 text-center text-zinc-400 italic">
            <span className="text-2xl mb-1">📊</span>
            <p className="text-xs font-bold text-zinc-500">
              {isEn
                ? "Need more data to find correlations. Keep logging your activity for a few more weeks!"
                : "Butuh lebih banyak data untuk menemukan pola. Terus catat aktivitasmu selama beberapa minggu lagi!"}
            </p>
          </div>
        )}
      </div>
