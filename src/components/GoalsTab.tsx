...
export default function GoalsTab({ state, onChange, showToast }: GoalsTabProps) {
  ...
  const [linkedType, setLinkedType] = useState<"workout_sessions" | "workout_distance_km" | "expense_savings">("workout_sessions");
  ...
  const handleAutoCalculateDeadline = (targetStr: string, progressStr: string, speedStr: string) => {
    ...
    if (speedVal > 0 && targetVal > progVal) {
      ...
      setGoalDeadline(toLocalDateStr(estDate));
  ...
  const handleOpenAdd = () => {
    ...
    setGoalSpeed("");
    setIsLinked(false);
    setLinkedType("workout_sessions");
    setWorkoutTypeFilter("");
    const today = new Date();
    // Default deadline: 30 days from now
    const defaultDate = toLocalDateStr(new Date(today.setDate(today.getDate() + 30)));
  ...
  const handleOpenEdit = (g: Goal) => {
    ...
    setIsLinked(!!g.linkedMetric);
    setLinkedType(g.linkedMetric?.type || "workout_sessions");
    setWorkoutTypeFilter(g.linkedMetric?.workoutTypeFilter || "");
    setIsEditOpen(true);
  };
    ...
    <div className="space-y-6 animate-fade-in text-zinc-700 pb-4">
                ...
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex flex-wrap gap-1 items-center">
                      ...
                      {g.linkedMetric && (
                        <span className="text-[9px] px-2.5 py-0.5 rounded-full border font-black uppercase bg-blue-50 border-blue-200 text-blue-600 tracking-wider">
                          🔗 Otomatis
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-zinc-100 mt-4">
                    ...
                    <label className="flex items-center gap-1.5 select-none cursor-pointer">
                      <input
                        ...
                        disabled={!!g.linkedMetric}
                        className="w-4 h-4 rounded-md border-zinc-300 accent-emerald-500 disabled:opacity-50"
                      />
      ...
      <Modal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="➕ Tambah Goal Baru">
        <div className="space-y-4">
          ...
          {/* Linked Metric Automation Section */}
          <div className="border-t border-zinc-100 pt-4 mt-2">
            <label className="flex items-center gap-2 select-none cursor-pointer mb-3">
              <input
                type="checkbox"
                checked={isLinked}
                onChange={(e) => setIsLinked(e.target.checked)}
                className="w-4 h-4 rounded-md border-zinc-300 accent-[#C9A84C]"
              />
              <span className="text-xs font-bold text-zinc-700">
                🔗 Hubungkan goal ini ke aktivitas otomatis
              </span>
            </label>

            {isLinked && (
              <div className="space-y-3 p-3.5 bg-zinc-50 rounded-2xl border border-zinc-100 animate-fade-in">
                <div>
                  <label className="block text-[9px] text-zinc-400 uppercase tracking-widest font-black mb-1.5">
                    Opsi Aktivitas Otomatis
                  </label>
                  <select
                    value={linkedType}
                    onChange={(e) => setLinkedType(e.target.value as any)}
                    className="w-full bg-white border border-zinc-200 rounded-xl px-3 py-2 text-xs outline-none focus:border-[#C9A84C]"
                  >
                    <option value="workout_sessions">Jumlah Sesi Workout</option>
                    <option value="workout_distance_km">Jarak Lari/Cardio (Km)</option>
                    <option value="expense_savings">Penghematan Bulanan</option>
                  </select>
                </div>

                {(linkedType === "workout_sessions" || linkedType === "workout_distance_km") && (
                  <div>
                    <label className="block text-[9px] text-zinc-400 uppercase tracking-widest font-black mb-1.5">
                      Filter tipe workout (kosongkan = semua)
                    </label>
                    <select
                    ...
                    >
                      ...
                      {WTYPES.filter(t => t.id !== "rest").map((t) => (
                ...
                <p className="text-[10px] text-zinc-400 font-medium leading-relaxed">
                  Progress goal ini akan otomatis bertambah berdasarkan aktivitas yang kamu catat di tab Workout/Pengeluaran — tidak perlu update manual.
                </p>
              </div>
            )}
          </div>
